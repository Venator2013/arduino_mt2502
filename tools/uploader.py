...
class MTKModem(object):
    ...
    def open(self, port):
        self.ser = serial.Serial(
            port, 115200, timeout=5, dsrdtr=True, rtscts=True)
    ...
    def ListFiles(self, path):
        ...
        fileList = self.SendCommandResult(
            'AT+EFSL="' + filenamePath + '"', True)
        ...
        for item in fileList:
            ...
            for value in quoted.findall(item):
                ...
                logging.debug("{0:30s} {1:10s} {2:5s}".format(
                    myname.decode('utf-16-be'), filesize, attrib))
        logging.debug("")
    ...
    def clearSketches(self, path):
        ...
        fileList = self.SendCommandResult(
            'AT+EFSL="' + binascii.hexlify(path.encode("utf-16-be")).decode() + '"', True)
        if len(fileList) == 0:
            return
        for item in fileList:
            diritem = item.split(",")
...
def main():
    parser = argparse.ArgumentParser(
        description='Push Application Utility', prog='uploader')
    parser.add_argument(
        '-port', '-p', help='Serial port device', default='ttyACM1')
    parser.add_argument('-app', help='File for uploading', default='demo.ino')
    parser.add_argument(
        '-clear', help='Clear old sketches in MRE folder', action='store_true')
    parser.add_argument(
        '-verbose', '-v', help='Verbose output', action='count')
